...
trait Bootstrapper {
  ...
  val isDevMode: Boolean = Props.getBoolean("devmode", default = false)

  def getDeviceId: Option[String] = None
  def getCurrentExpoUID: Option[String] = None
  def getCurrentExpoToken: Option[String] = None
  def getHostStr(secure: Boolean = false): StringBuilder = if (secure) {
  ...
  def init(initParam: Option[ClosableWrapper]): Unit
