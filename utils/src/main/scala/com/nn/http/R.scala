...
*/
  ...
  private final class SFuncHolder(val func: String => Any, val owner: Option[String]) extends AFuncHolder with Serializable{
    def this(func: String => Any) = this (func, None)
...
trait R {
  ...
  //private val _responseHeaders = new ThreadLocalWrapper[ResponseInfoHolder]
  //private val _responseCookies = new ThreadLocalWrapper[CookieHolder]
  private val _lifeTime = new RichThreadLocal[Boolean]
  private val autoCleanUp = new RichThreadLocal[Boolean]
  //private val _oneShot = new RichThreadLocal[Boolean]
  private val _disableTestFuncNames = new RichThreadLocal[Boolean]
...
*/
  ...
  def build_num = if (Props.mode == Props.Modes.prod) "0" else Helpers.nextFuncName
  ...
  //def currentCometActor: Option[CometActor] = CurrentCometActor.box
...
*/
   ...
   */
  /*
  def runExceptionHandlers(req: HttpRequest, orig: Throwable): Option[TheResponse] = {
    R.assertExceptionThrown() 
    tryo{(t: Throwable) => 
...
*/
  ...
  //Option.legacyNullTest(_sessionInfo.value)
   ...
   * that class
   */
  def snippetForClass(cls: String): Option[DispatchSnippet] =
  ...
  /*
  private var _queryAnalyzer: List[(Option[HttpRequest], Long,
   ...
   * @see # queryLog
   */
  def addAnalyzer(f: (Option[HttpRequest], Long,
...
*/
        ...
        try {
          wrapQuery {
            ...
            session match {
              case Some(s) => {
                if (s.stateful_?)
                  Rules.earlyInStateful.toList.foreach(_(req))
                else
                  Rules.earlyInStateless.toList.foreach(_(req))
              }
              case _ =>
            }
            f
  ...
  */
  ...
  /*
  private def doStatefulRewrite(old: HttpRequest): HttpRequest = {
...
*/
  ...
  */
  ...
  def addFunctionMap(name: String, value: AFuncHolder): Unit = {
    testFunctionMap {
      (autoCleanUp.value/*, nullTest(_oneShot.value)*/) match {
        case true =>
        ...
        /*
                case (_, Some(true)) => {
                  updateFunctionMap(name,
                    new R.ProxyFuncHolder(value) {
                      var shot = false
                      lazy val theFuture: Future[Any] = {
                        R.session.map(_.removeFunction(name))
                        val future: Future[Any] = new Future
  ...
  */
  ...
  //Do not need forms
  /*
  private[http] object _formGroup extends TransientRequestVar[Option[Int]](None)
  ...
  def formFuncName: String = if (Props.testMode && !disableTestFuncNames_?) {
    ...
    "f" + prefix + "_" + hashHex((new Exception).getStackTrace.toList.filter(notLiftOrScala).take(2).map(_.toString).mkString(","))
  } else {
    _formGroup.is match {
      case Some(x) => nextFuncName(x.toLong * 100000L)
      case _ => nextFuncName
    }
  }
  ...
  */
  ...
  def mapFuncToURI(uri: String, f: () => Unit): String = {
    session map (_ attachRedirectFunc (uri, nullTest(f))) getOrElse uri
  }
  * 
  */
   ...
   */
  def contextFuncBuilder(f: R.AFuncHolder): R.AFuncHolder = R.session match {
    case Some(s) => s.contextFuncBuilder(f)
    case _ => f
  }
