...
import java.util.{ArrayList => JavaArrayList, Iterator => JavaIterator}
...
sealed abstract class EmptyBox extends Box[Nothing] with Serializable {
  ...
  override def ?~(msg: => String): FailureBox = FailureBox(msg, Empty, Empty)
 ...
 */
object FailureBox {
  def apply(msg: String) = new FailureBox(msg, Empty, Empty)
}
 ...
 */
sealed case class FailureBox(msg: String, exception: Box[Throwable], chain: Box[FailureBox]) extends EmptyBox{
  type A = Nothing
  ...
  private def chainList: List[FailureBox] = chain match {
   ...
   */
  def failureChain: List[FailureBox] =
    this :: chain.toList.flatMap(_.failureChain)

  def messageChain: String = (this :: chainList).map(_.msg).mkString(" <- ")

  override def equals(other: Any): Boolean = (this, other) match {
    case (FailureBox(x, y, z), FailureBox(x1, y1, z1)) => x == x1 && y == y1 && z == z1
    case (x, y: AnyRef) => x eq y
    case _ => false
  }

  override def ?~(msg: => String): FailureBox = this

  override def ?~!(msg: => String): FailureBox = FailureBox(msg, Empty, Full(this))
}